            ...
            <div class="card-body p-5">
                ...
                <div>
                    ...
                    <div class="row filter-container" style="border: 1px solid black;">
                        <div class="col-3 mt-3 mb-2  pe-0">
                        ...
                        <div class="col-3 mt-3 mb-2  pe-1 ">
                            <label>HOA Group</label>
                            <select id="hoagroup_filter" class="form-select form-select-sm"
                                aria-label="Default select example">
                                <option value="all" selected>All</option>
                                @foreach ($hoa_groups as $hoa_group)
                                    <option value="{{ $hoa_group->name }}">{{ $hoa_group->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-3 mt-3 mb-2 pe-0 ">
                    ...
                    <table class="table table-bordered " id="myTable">
                        <thead class="bg-primary text-white">
                            <tr>
                                ...
                                <th scope="col " style="font-size:9px">Member Type</th>
                                <th scope="col " style="font-size:9px">HOA Group</th>
                        ...
                        <tbody>
                            @foreach ($sticker_prices_matrix as $index => $spc)
                                <tr data-category="{{ $spc->category_name }}">
                                    ...
                                    <td class="text-nowrap" style="font-size: 9px">
                                        <a href="{{ route('spc3-hoa-group') }}" target="_blank">{{ $spc->hoagroup_name ?? '' }}</a>
                                    </td>
                            ...
                            <div class="card" style="border-radius: 10px;">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-3">
                                            <div class="mb-3">
                                                <label for="" class="form-label">HOA Group: </label>
                                                {{-- <select name="hoa[]" id="hoa" class="form-select form-select-sm  mb-3 hoa-add" aria-label=".form-select form-select-sm-lg example" multiple> --}}
                                                <select name="hoa_group" id="hoa_group"
                                                    class="form-select form-select-sm  mb-3 "
                                                    aria-label=".form-select form-select-sm-lg example">
                                                    <option value="">---</option>
                                                    {{-- @foreach ($hoas as $hoa)
                                            ...
                                            @endforeach --}}
                                                    @foreach ($hoa_groups as $hoa_group)
                                                        <option value="{{ $hoa_group->group_id }}">{{ $hoa_group->name }}</option>
                                                    @endforeach
                                                </select>
                                                </select>
                                            </div>
                                        ...
                                        <div class="col-md-3">
                                            <div class="mb-3">
                                                <label for="" class="form-label" style="font-size: 12px;">Vehicle Type: </label>
...
@section('links_js')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // 11/12/24 Filter Bug Fix - JY
            $('#category_filter, #sub_category_filter, #hoa_filter, #hoatype_filter, #vehicle_ownership_filter, #vehicle_filter, #hoagroup_filter')
                .on('change', function() {
                    ...
                    var hoaGroup = $('#hoagroup_filter').val();
                    ...
                    table.columns(4).search(hoatype !== 'all' ? '^' + escapeRegExp(hoatype) + '$' : '', true,
                        false).draw();
                    table.columns(6).search(vos !== 'all' ? '^' + escapeRegExp(vos) + '$' : '', true, false)
                        .draw();
                    table.columns(7).search(vehicleType !== 'all' ? '^' + escapeRegExp(vehicleType) + '$' : '',
                        true,
                        false).draw();

                    table.columns(5).search(hoaGroup !== 'all' ? '^' + escapeRegExp(hoaGroup) + '$' : '', true,
                        false).draw();
                });
        });
        ...
        function categoryChange(id,selectedId) {
            ...
            // $('#hoa option').hide();
            $(`#hoa option`).remove();
            $('#mySelect').trigger('change');  // Refresh Select2
            hoas.forEach(data => {
                if(id == 1){
                    if(data.type == 0 || data.type == 1){
                        // $(`#hoa option[value="${data.id}"]`).show();
                        // $(`#hoa option[value="${data.id}"]`).prop('disabled',false);
                        $(`#hoa`).append(`<option value="${data.id}">${data.name}</option>`)
                    }
                    else{
                        // $(`#hoa option[value="${data.id}"]`).hide();
                        // $(`#hoa option[value="${data.id}"]`).prop('disabled',true);
                        $(`#hoa option[value="${data.id}"]`)
                        .remove();
                    }
                    // if(data.type !== 0 && data.type !== 1){
                    ...
                    // if(data.type == 0 || data.type == 1){
                    //     console.log($(`#hoa option[value="${data.id}"]`).val(),'data1')
                    //     $(`#hoa option[value="${data.id}"]`).remove();
                    // }
                }
            })
        ...
        $(document).ready(function() {
            $('#sellingPrice, #stickerPrice, #vatAmount, #surcharge').on('input', function() {
                ...
                // If any of the values in sellingPrice, stickerPrice, or vatAmount is not a number
                // Clear the security, roadMaintFee, and totalC fields, and return
                ...
                // If sellingPrice or stickerPrice is 0, clear the security, roadMaintFee, and totalC fields, and return
                ...
                // If surcharge is not a number, set it to 0
                if (isNaN(surcharge)) {
                    surcharge = 0;
                }
                var subcatId = $('#sub_category').val();
                ...
                // Comment this for meantime - bug JY
                // If subcategory is tenant, business owner, property owner then security is equal to total amount
                // if (subcatId == 2 || subcatId == 3 || subcatId == 4) {
                //     security = totalAmount
                // }
                // // otherwise, security is the minimum of total amount and 1000
                // else {
        ...
        function getPrice(id) {
            ...
            $.get('/get-price-spc/' + id, function(price) {
                ...
                $('#hoa_group').val(price[0].hoa_group_id);
                ...
                // console.log(price[0]);
                 setTimeout(() => {
                    if (!price[0].isOR) {
                        ...
                        $('#orType').prop('checked', false).trigger('change');
                        $('#crType').prop('checked', true).trigger('change');
                        ...
                        $('#orType').prop('checked', true).trigger('change');
                        $('#crType').prop('checked', false).trigger('change');

                        console.log($('#orType').is(':checked'));
                ...
                $('#sellingPrice, #stickerPrice, #vatAmount, #surcharge').trigger('input');
            })
        };
